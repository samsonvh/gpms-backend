...
namespace GPMS.Backend.Services.Utils.Validators
{
    public class ProductDefinitionInputDTOValidator : AbstractValidator<ProductDefinitionInputDTO>
    {
        public ProductDefinitionInputDTOValidator()
        {
            ...
            RuleFor(inputDTO => inputDTO.Category).NotNull().NotEmpty().WithMessage("Category is required");

            RuleFor(inputDTO => inputDTO.SemiFinishedProducts.Count).GreaterThan(0).WithMessage("Semi finished product list is required");

            RuleFor(inputDTO => inputDTO.Materials.Count).GreaterThan(0).WithMessage("Material is required");
        }
    }
}